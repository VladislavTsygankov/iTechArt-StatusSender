...
import { FROM_STATUS_SENDER } from '../constants/mailgun';

const sendNotification = ({ username, timeForSend, projectName, reminderValue }) => {
  return sendMail({
    from: FROM_STATUS_SENDER,
    to: `${username}@itechart-group.com`,
    subject: `${reminderValue} left before send ${projectName} status`,
    text: `Please, do not forget about your status on ${projectName}
     project which will send at ${timeForSend}`,
  });
};

const sendProjectStatus = ({ name, addressees, copyAddressees, greeting, signature }) => {
  return sendMail({
    from: FROM_STATUS_SENDER,
    to: getValidAddresseesString(addressees),
    cc: getValidAddresseesString(copyAddressees),
    subject: `${name} project status on ${momentService
      .getCurrentUTCDate()
      .date.format('DD/MM?YYYY')}`,
    text: createTextMessage({ greeting, signature }),
  });
};
